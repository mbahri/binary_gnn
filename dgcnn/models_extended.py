...
class BinDGCNN_RF(nn.Module):
    ...
    def __init__(self, args, output_channels=40, max_pool=True, avg_pool=True):
        ...
        self.conv1 = mc.BinConv2dBlockBN(
            6,
            64,
        ...
        self.conv2 = mc.BinConv2dBlockBN(
            64 * 2,
            64,
            bn_momentum=self.bn_momentum,
            binary_weights=self.binary_weights,
        ...
        self.conv3 = mc.BinConv2dBlockBN(
            64 * 2,
            128,
            bn_momentum=self.bn_momentum,
            binary_weights=self.binary_weights,
        ...
        self.conv4 = mc.BinConv2dBlockBN(
            128 * 2,
            256,
            bn_momentum=self.bn_momentum,
            binary_weights=self.binary_weights,
        ...
        self.conv5 = mc.BinConv1dBlockBN(
            512,
            args.emb_dims,
            bn_momentum=self.bn_momentum,
            binary_weights=self.binary_weights,
        ...
        self.linear1 = mc.BinLinearBlockBN(
            args.emb_dims * embd_in_scale,
            512,
            bn_momentum=self.bn_momentum,
            binary_weights=self.binary_weights,
            binary_inputs=self.binary_inputs,
        ...
        self.linear2 = mc.BinLinearBlockBN(
            512,
            256,
            bn_momentum=self.bn_momentum,
            binary_weights=self.binary_weights,
        ...
        self.linear3 = mc.BinLinearBlockBN(
            256,
            output_channels,
            bn_momentum=self.bn_momentum,
...
class BinDGCNN_BF1(nn.Module):
    def __init__(self, args, output_channels=40, max_pool=True, avg_pool=True):
        ...
        self.conv1 = nn.Sequential(
            binary.BinConv2d(
                6, 64, kernel_size=1, bias=False, binary_weights=self.binary_weights
            ),
    ...
    def forward(self, x0):
        ...
        x, ei1 = mc.get_graph_feature_lsp(
            x1, k=self.k, knn=self.knn_op, edge_op=self.edge_op
        )
        x = self.conv2(x)
        x2 = x.max(dim=-1, keepdim=False)[0]
        x2 = self.quantizer(self.bn2(x2))

        x, ei2 = mc.get_graph_feature_lsp(
            x2, k=self.k, knn=self.knn_op, edge_op=self.edge_op
        )
        x = self.conv3(x)
        x3 = x.max(dim=-1, keepdim=False)[0]
        x3 = self.quantizer(self.bn3(x3))

        x, ei3 = mc.get_graph_feature_lsp(
            x3, k=self.k, knn=self.knn_op, edge_op=self.edge_op
        )
        x = self.conv4(x)
        x4 = x.max(dim=-1, keepdim=False)[0]
...
class BinDGCNN_BF1_BAL(BinDGCNN_BF1):
    def __init__(self, args, output_channels=40, max_pool=True, avg_pool=True):
        ...
        self.linear1 = mc.BinLinearBlockNoBN(
            args.emb_dims * embd_in_scale,
            512,
            bn_momentum=self.bn_momentum,
            binary_weights=self.binary_weights,
